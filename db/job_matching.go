// Package db 求職撮合（10.15）：無職且鎂低於閾值的 NPC 與有職缺的場所匹配，寫入 assignment 與排班。對齊討論 002。
package db
...
func RunJobMatchingTick(db *sql.DB, g *RoomGraph, p JobMatchParams) []string {
	if p.MaxPerVenue <= 0 {
		p.MaxPerVenue = 2
	}
	...
	var added []string
	ids, err := GetNPCIDsWithRoom(db)
	if err != nil || len(ids) == 0 {
		return added
	}
	var unassigned []string
	for _, entityID := range ids {
		list, _ := GetAssignmentsForEntity(db, entityID)
		if len(list) > 0 {
			continue
		}
		c, _ := GetEntity(db, entityID)
		if c == nil {
			continue
		}
	...
	venueIDs, err := GetAllVenueIDs(db)
	if err != nil || len(venueIDs) == 0 {
		return added
	}
	var slots []string
	for _, vid := range venueIDs {
		n, _ := GetAssignmentCountByVenue(db, vid)
		limit := GetVenueMaxStaff(db, vid, p.MaxPerVenue)
		for k := 0; k < limit-n; k++ {
	...
	if g != nil {
		...
		for _, venueID := range slots {
			...
			for i, entityID := range unassigned {
				current, _ := GetEntityRoom(db, entityID)
				if current == "" {
					continue
				}
			...
			if doAssign(db, entityID, venueID, spawnRoom) {
		...
		for i := 0; i < n; i++ {
			if doAssign(db, unassigned[i], slots[i], spawnRoom) {
...
func doAssign(db *sql.DB, entityID, venueID, spawnRoom string) bool {
	occupation := GetFirstOccupationIDForVenue(db, venueID)
	if occupation == "" {
		occupation = DefaultOccupationForHire
	}
	if InsertAssignment(db, entityID, occupation, venueID, "job_matching") != nil {
		return false
	}
	roomIDs, _ := GetRoomIDsForVenue(db, venueID)
	var workRoom string
	...
	if workRoom != "" && spawnRoom != "" {
		_ = InsertSchedule(db, entityID, workRoom, spawnRoom, defaultShiftStart, defaultShiftEnd)
	}
	return true
}
