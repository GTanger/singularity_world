...
| **合併檔**（`data/rooms.json`，可選） | **一房一行** | 執行 `node tools/js/merge-rooms-one-per-line.js` 自 `data/rooms/` 產生；**執行期房間載入不依賴此檔**（伺服器讀一房一檔目錄）。僅供瀏覽、比對或舊工具鏈。 |
...
- **id 規則**：前綴第一層 = zone 的英文代碼（例：**浮生城**→`citylife`）；前綴第二層 = 該建築/場所的英文名（例：綠意別墅→green）。同一建築內格子 id 常為 `zone_place_` 開頭，例：`lifestreet_green_大廳`（僅作命名範例，與現存檔無關亦可）。
...
| **zone** | string | 選 | 所屬區域（如浮生城寫 `citylife`），用於分區與子目錄分類。 |
...
- 第一層：依專案習慣分子資料夾（現行以 **`editor/`** 為浮生城主體；其餘 zone 可自行增刪資料夾）。
