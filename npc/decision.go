// Package npc：決策引擎（腦）Decide(state, context) -> Intent。
...
type DecisionState struct {
	...
	// InventoryTotalQty 背包可售物總數量（用於 IntentTrade／行腳兜售）；不含 0。
	InventoryTotalQty int
}
...
// BuildDecisionState 從 store 組裝 NPC 的決策狀態。
func BuildDecisionState(entityID string) (DecisionState, error) {
	var s DecisionState
	s.EntityID = entityID
	...
	if c.SoulSeed != nil {
		s.Personality = db.ExpandSoulSeedToPersonality(*c.SoulSeed)
		s.HasPersonality = true
	}
	s.CurrentRoomID, _ = db.GetEntityRoom(entityID)
	if c.Inventory != "" && c.Inventory != "[]" {
		ir := db.GetInventory(c.Inventory, c.Vit)
		for _, it := range ir.Items {
			s.InventoryTotalQty += it.Qty
		}
	}
	// 最近一筆 EvtTalk 事件時間戳作為社交衰減起點
	for _, e := range db.GetRecentEvents(entityID, 10) {
		if e.EventType == db.EvtTalk {
			s.LastTalkAt = e.At
			break
		}
	}
...
// BuildDecisionContext 從 RoomGraph 與 store 組裝當前房間周邊情境（maxDist 步內）。
func BuildDecisionContext(g *db.RoomGraph, currentRoomID string, maxDist int) DecisionContext {
	if maxDist <= 0 {
		maxDist = 20
	}
	var ctx DecisionContext
	ctx.RoomTags = g.RoomTags(currentRoomID)
	venueRooms, _ := db.GetAllVenueRoomIDs()
...
func filterRoomsWithinDist(g *db.RoomGraph, origin string, candidates []string, maxDist int) []string {
...
func personalityWeightedSelect(candidates []candidate, p db.Personality, hasPers bool) Intent {
	if len(candidates) == 0 {
...
func Decide(state DecisionState, ctx DecisionContext) Intent {
	...
	// ── 兜售／行腳商：背包有貨且生存壓力時，往熱鬧處換鎂（討論 002：不必 assignment）
	if state.InventoryTotalQty > 0 && survival > 0.05 {
		if len(ctx.SocialOrGateInRange) > 0 || hasTag(ctx.RoomTags, "gate", "social") {
			score := survival * 0.45
			if hasPers {
			...
			if score > 0.05 {
				...
				cands = append(cands, candidate{
					intent: Intent{Type: IntentTrade, TargetRoomID: target},
					weight: score,
				})
			}
		}
...
func ResolveBrainPath(g *db.RoomGraph, intent Intent, currentRoomID string, ctx *DecisionContext, maxDist int) []string {
