...
func AdvanceMovement(mapsPath string, now int64) ([]MovedResult, error) {
	list, err := db.GetMovingEntities()
	...
	for _, e := range list {
		...
		if e.X == tx && e.Y == ty {
			_ = db.UpdatePosition(e.ID, tx, ty)
			_ = event.Append(now, e.ID, event.TypeArrived, "")
		...
		nx, ny := e.X, e.Y
		for s := 0; s < steps; s++ {
			view, err := GetChunkAndEntitiesInView(nx, ny, mapsPath)
			if err != nil {
				break
			}
			dx := sign(tx - nx)
			...
			if !view.CanMoveToWorld(nextX, nextY) {
				_ = event.Append(now, e.ID, event.TypeBlocked, "")
				_ = db.UpdatePosition(e.ID, nx, ny)
				break
			}
			nx, ny = nextX, nextY
			if nx == tx && ny == ty {
		...
		if nx != e.X || ny != e.Y {
			_ = db.UpdatePositionOnly(e.ID, nx, ny)
			out = append(out, MovedResult{ID: e.ID, X: nx, Y: ny})
		}
		if nx == tx && ny == ty {
			_ = db.UpdatePosition(e.ID, tx, ty)
			_ = event.Append(now, e.ID, event.TypeArrived, "")
			if other, _ := EntityAt(tx, ty, e.ID); other != nil {
				_ = event.Append(now, e.ID, event.TypeContact, other.ID)
			}
		}
	}
	return out, nil
...
func EntityAt(x, y int, excludeID string) (*entity.Character, error) {
	list, err := db.GetEntitiesInBox(x, x, y, y, "")
	if err != nil {
		return nil, err
	}
	for _, e := range list {
