...
- **實作現狀**：伺服器以 **Ollama** 為單一對話後端時，`OllamaModel` 為**可替換字串**（`OLLAMA_MODEL` 或 `config.DefaultServer()` 預設值）；**預設 tag 會隨專案迭代改動**，與「官方 Library 上某一固定名」無必然綁定。
- **營運習慣**：本機常見作法是**關注 Ollama 新小模型、pull 試跑**；若體感更好，改環境變數即可，**不必**視程式預設為永恆。
- **與 NPC 活化對齊**：見 [NPC活化系統.md](../NPC活化系統.md) §零點五。
