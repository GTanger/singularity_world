...
| 階段 | 名稱 | 核心效果 | 狀態 | 備註 |
|------|------|---------|:----:|------|
| **A** | 鎂消耗（經濟循環） | NPC 會餓，馬斯洛鏈激活 | ✅ | `db/npc_expense.go`、main 每日扣鎂、EvtBroke／DispDaily |
| **B** | Brain 停留時間 | NPC 到達後停留，不亂飛 | ✅ | `npc_movement.go` Tick 先 computeStay 再清空、MoveBrain case |
...
| **soul_seed 展開** | 三軸常數、BaseStats、OriginSentence、Personality、GetPersonalityForEntity、TopologyCosts | S1–S6：`db/entity.go`、`db/topology.go` | ✅ |
...
| 區塊 | 已實作檔案／改動 |
|------|------------------|
| **A** | `db/npc_expense.go`（DailyExpenseBase、DeductDailyExpense）、`db/npc_events.go`（LogNPCEvent）、`db/disposition.go`（常數＋AdjustDisposition）、main 每遊戲日 DeductDailyExpense |
| **B** | `db/npc_movement.go`：Tick 內先 computeStay 再清空 LastIntent；computeStay 新增 MoveBrain 分支（依意圖停留 1–5 遊戲小時） |
| **C** | `db/decision.go`：candidate、personalityWeightedSelect；Decide 生存／安定分支改為候選＋性格加權 |
| **D** | `db/npc_events.go`（GetRecentEvents）、store.RecentByEntity；main applyBrainArrivalEffects 各 case 加 LogNPCEvent |
| **E** | store.Entity / entity.Character 加 Disposition；db/disposition.go 實作 AdjustDisposition、GetDisposition；behavior PickIdleEmote(disposition)；main 各效果處呼叫 AdjustDisposition |
| **F** | `db/npc_social.go`（PickMicroInteraction、getNPCNamesInRoom）；main 閒置 tick 前對 playerRooms 觸發微互動（15%） |
| **G** | `db/backstory.go`（BuildIdentity、personalityToSentence）；依職稱、場所、性格、心境、最近 3 筆事件組 identity |
| **H** | store：ArchivalEntry、Archival、loadArchival、AppendArchival、GetArchivalByEntity；`db/archival.go`（InsertArchival、SearchArchival 簡易版） |
| **I** | `ai/talk.go`（CallAITalk stub）；`db/dialogue.go` PickStyleExamples；server ClientMsg.PlayerInput；handler Talk：backstory／snippets／styleExamples → CallAITalk，fallback buildTalkNarrative，會後 InsertArchival |

**NPC 池（可設定總量＋定時補滿）**：`config` 新增 `NPCPoolSize`、`NPCSpawnIntervalSec`（env：`NPC_POOL_SIZE`、`NPC_SPAWN_INTERVAL_SEC`）；`db/npc.go` 新增 `SpawnOneNPCFromPool(db, spawnRoomID)`；main 每 `NPCSpawnIntervalSec` 秒檢查「有房間的 NPC 數」< 池量則生成一名並註冊腦驅動。池滿後若有 NPC 死亡／移除，下一輪檢查會自動補一名。
