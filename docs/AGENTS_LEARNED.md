...
- **資源點採集（2026-04-08 v1.2 Gemini 修訂）**：核心為**「脈衝式收割 (Pulse Harvest)」**。地格無人時密度恆為 1.0 並累積富態位能（電荷）；人來則磁場排開輻射導致密度下降、增生停止。最優解為：進人、快速收割已積累電荷、能量枯竭後撤離讓環境回溫。產出為無定性白板材料，品階於剝名決定。**資料權威嚴格鎖定 PostgreSQL，禁止執行期 JSON 依賴。**見 `docs/design/資源點實作規範.md`。
