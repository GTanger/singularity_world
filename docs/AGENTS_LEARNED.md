...
- **資源點採集（2026-04-08 v1.3 Gemini 修訂）**：依循**「限制框架連續性」**。資源點並非無中生有，而是環境原生種（水稻、鐵礦等）在 Token 輻射下的富態增生；環境原本長什麼、富態化後仍沿用該框架（沙漠不產水稻）。採集為脈衝式位能坍縮，獲取具體富態物種實體。產物品類由環境框架釘死，品質與詞元則由後續剝名決定。核心數據威權鎖定 PostgreSQL。見 `docs/design/資源點實作規範.md`。
