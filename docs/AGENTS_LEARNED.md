...
- **資源點採集（2026-04-07 定案）**：MVP 階段僅處理**「有念生物被動採集」**（選項 1）。玩家或 NPC 進入資源地格後切換至 `Gathering` 狀態，由伺服器 tick 自動結算產出。採集者（有念生物）會壓低該格 Token 密度（密度公式：`1.0 - (人數 * 0.1)`），進而降低再生與產量。機械自動採集（選項 2）因涉及「自主意識進化」風險，暫不處理。產出僅限白板材料，品階於剝名時定。**資料權威一律存放於 PostgreSQL（`resource_pools`, `resource_nodes`），執行期嚴禁依賴 JSON。**見 `docs/design/資源點實作規範.md`。
