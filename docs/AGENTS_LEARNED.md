...
- **資源點採集（2026-04-08 設計者定案）**：Token 游離輻射行為類似流體——人進場域擠開等體積 Token（體積位移），念場向外相斥進一步壓低濃度（場效應），人離開密度即時回填無冷卻。每個資源個體有成熟度 0→1（完全變態質），到 1 才可採收；採收後歸零重新生長。資源個體為獨立可互動物件（MUD 物件制），成熟即出現、採完消失。hex 格 = 場域，密度為 hex 級共享，所有個體成長速率統一受影響。被動放置：玩家進入採集狀態持續作業。**Gemini v1.2~v1.5 已 revert 至 v1.1**（砍公式、加未定案欄位、偏離實作規格）。見 `docs/design/資源點與礦區設計.md`、`docs/design/資源點實作規範.md`。
