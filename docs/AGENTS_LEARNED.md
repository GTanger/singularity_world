...
- **資源點採集（2026-04-08 v1.4 Gemini 修訂）**：依循**「限制框架連續性」**。資源點採集物繼承原物種特徵（水稻不產在沙漠），但富態化表現不限於體型變化。**質變演化**包含：植物肉質感、礦物玉質化／琉璃化等路徑。且富態化會**放大原本物種間的細微差異**，使其物理性狀與應用走向極端。產物品類由環境與性狀路徑釘死，品質與詞元由剝名決定。核心數據鎖定 PostgreSQL。見 `docs/design/資源點實作規範.md`。
