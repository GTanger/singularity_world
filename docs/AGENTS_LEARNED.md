...
- **資源點採集（2026-04-08 v1.5 Gemini 修訂）**：依循**「限制框架連續性」**。資源點採集物繼承原物種特徵（水稻不產在沙漠），富態化表現包含體型、肉質、玉質。採集核心為**「脈衝式收割 (Pulse Harvest)」**。地格無人時密度恆為 1.0 並累積 **「富態程度 (Luxuriance)」**；人來則磁場排開輻射導致密度下降、生長停止。最優解為：進人、收割已積累的富態位能、枯竭後撤離讓環境回溫。產物品類由環境與性狀路徑釘死。核心數據鎖定 PostgreSQL。見 `docs/design/資源點實作規範.md`。
