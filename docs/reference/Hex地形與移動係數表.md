...
- **move_cost 是地形（地理）阻力，與地上物無關**。樹、礦等地上物是格內實體，佔位阻擋路線，玩家須繞行。詳見 `map_terrain_world.md` §4.6。

## 移動速度公式（2026-04-10 定案）

```
base_speed = 60 px/s（真實步行 ×2 倍率）
actual_speed = base_speed / move_cost
```

玩家點擊目標位置後字圓以 `actual_speed` 直線移動，撞到不可通行實體即停止。
**無自動尋路**——系統不替玩家規劃路線，玩家自行判斷繞行。

詳見 `map_terrain_world.md` §4.4–4.5。

## 目前路徑計算的注意事項

目前 `HexGrid::find_path()` 是 BFS（按步數最短），**尚未使用 `move_cost` 作加權最短路**。
此函式用於 NPC 尋路，非玩家移動（玩家為直線移動、無尋路）。
