...
| `data/templates/llm_prompts.json` | 給 LLM 的 system／user 區塊（玩家↔NPC、NPC↔NPC）、世界現象段落、meta 過濾、折疊分隔符。由 `ai/prompts` 延遲載入。 |
...
**原則**：可編輯內容放在 JSON；後端只做載入、格式化與接線。註解僅供人讀，不等於送進模型的提示詞。
