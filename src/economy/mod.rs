// economy 模組 — 經濟 tick 與鎂轉帳門面，對齊 Go economy/。

use std::time::Duration;

/// 將 `from_id` 的 `amount` 鎂轉給 `to_id`；實作於 `db`（store）。對齊 Go `economy.TransferMagnesium`。
...
/// 對齊 Go `economy.Run`（goroutine + ticker）；此處使用 `tokio::spawn` + `interval`。
