// NPC↔NPC 執行緒／雙元關係 store 門面，對齊 Go db 層對 `npc_thread`／`npc_dyad` 的存取。
...
/// 傳聞池時間衰減與過期刪除（對齊 Go `DecayNpcRumors`）。
...
/// 重建傳聞 digest 摘要檔（對齊 Go `BuildNpcRumorDigest`）。
