// 場所與指派門面，對齊 Go db/assignment.go。
...
/// 確保預設場所存在；現由 `venues.json` 於 store 初始化載入，此函式為 no-op（對齊 Go `SeedVenues`）。
pub fn seed_venues() -> anyhow::Result<()> {
    Ok(())
}

/// 新增一筆指派；同 entity+occupation+venue 已存在則忽略（對齊 Go `InsertAssignment`）。
...
/// 某實體的全部指派（對齊 Go `GetAssignmentsForEntity`）。
...
/// 依指派推導職稱；無指派則空字串（對齊 Go `GetNPCTitleFromAssignments`）。
#[must_use]
...
/// 移除某實體的全部指派（對齊 Go `RemoveAssignmentsForEntity`）。
...
/// 該場所職缺上限；0 用 `default_max`（對齊 Go `GetVenueMaxStaff`）。
#[must_use]
...
/// 該場所涵蓋的房間 ID（對齊 Go `GetRoomIDsForVenue`）。
...
/// 實體在指定房間時是否處於任一指派場所內（對齊 Go `EntityInVenueAtRoom`）。
...
/// 包含該房間的所有場所 ID（對齊 Go `GetVenueIDsForRoom`）。
...
/// 該場所目前的指派數量（對齊 Go `GetAssignmentCountByVenue`）。
...
/// 所有場所涵蓋的房間 ID（對齊 Go `GetAllVenueRoomIDs`）。
