...
/// 同步啟動 writer service——集中的 PG 寫入 queue，取代散落的 spawn thread
pub fn set_pool(pool: DbPool) {
    {
        let mut g = DB_POOL.write().unwrap_or_else(|e| e.into_inner());
...
pub fn pool() -> Option<Arc<DbPool>> {
    DB_POOL.read().unwrap_or_else(|e| e.into_inner()).clone()
}
